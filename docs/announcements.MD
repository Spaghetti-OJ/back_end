...
| 參數        | 說明           |
| ----------- | -------------- |
| `course_id` | 要查詢的課程 ID；公共討論區可使用 `1` |
...
- 公開討論區路徑可將 `course_id` 設為 `1` 代表系統公告（公開討論區）。
...
- 若該課程名稱為「公開討論區」，可直接存取（無須登入）。
- 公開討論區路徑可將 `course_id` 設為 `1` 代表公開討論區（系統公告）。

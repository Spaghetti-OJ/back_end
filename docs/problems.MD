...
沙盒在判題前會：
1. 下載 zip 檔案。
2. 呼叫 checksum 端點確認 MD5 是否一致（避免網路傳輸損毀）。
...
- 回傳欄位：
  - `checksum`: 與 /checksum 相同 MD5（方便比對）
  - `task_count`: 總測試對數量
...
echo "$CHECKSUM  testdata.zip" | md5sum -c
...
> 靜態分析驗證錯誤（啟用 forbid-functions 但未提供函數列表，預期 400/422）
```bash
curl -X PUT http://127.0.0.1:8000/problem/manage/$PROBLEM_ID \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \

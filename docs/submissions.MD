...
## 概述
本文檔描述了題解系統的四個核心API端點，用於管理問題的題解內容。
...
### 1. 發布題解
**POST** `/submissions/problem/{problemId}/solution/`
...
### 2. 取得題解列表
**GET** `/submissions/problem/{problemId}/solution/`
...
### 3. 修改題解
**PUT** `/submissions/problem/{problemId}/solution/{solutionId}/`
...
### 4. 刪除題解
**DELETE** `/submissions/problem/{problemId}/solution/{solutionId}/`
...
**POST** `/submissions/problem/{problemId}/solution/{solutionId}/like/` - 按讚
**DELETE** `/submissions/problem/{problemId}/solution/{solutionId}/like/` - 取消按讚
...
3. **標題長度**: 標題最多255個字元
4. **問題關聯**: 題解必須關聯到存在的問題
5. **課程權限**: 問題必須屬於某個課程，且操作者必須是該課程的教學人員
6. **狀態管理**: 創建的題解自動設為"已發布"狀態
7. **統計更新**: 查看題解時會自動增加瀏覽次數
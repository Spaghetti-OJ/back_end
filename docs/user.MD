...
**說明**：
使用目前登入狀態（Authorization Header 帶入 `Bearer <access>`）取得當前使用者的基本資料，包含 `user_id`、`username`、`email`、`role`、`email_verified`，以及使用者可管理的課程清單 `access_course`。
...
| `data.role`           | string | 使用者角色（如：`student`、`teacher`、`admin`） |
| `data.email_verified` | bool   | 是否已完成信箱驗證                            |
| `data.access_course`  | int[]  | 使用者具有管理權限（Teacher / TA）的課程 ID 清單     |

---

### 權限與說明補充

* `access_course` 會回傳目前使用者 **可管理的課程**（角色為 Teacher 或 TA）。
* 若使用者僅為課程學生，則此欄位會回傳空陣列 `[]`。
* 建立課程的教師會自動被加入該課程的 `Course_members`，並具有 `teacher` 權限。
...
> 使用說明：
>
> * 建議登入後（呼叫 `POST /auth/session/` 取得 access token）立刻呼叫一次 `GET /auth/me/`，
>   將回傳的 `user_id`、`username`、`email`、`role`、`email_verified` 與 `access_course`
>   存入前端全域狀態（如 Redux / Zustand / Vuex），作為後續顯示使用者資訊與判斷課程管理權限的依據。
> * 若收到 401，請確認 access token 是否過期，可搭配 `POST /auth/refresh/` 重新取得新的 access token。
...
**說明**：
使用者需同時輸入正確的 `username` 與 `email`。若兩者皆正確，且該帳號已綁定並**完成信箱驗證**，系統將寄出一封「重設密碼」信件至該帳號的信箱。信件中包含一組一次性且有時效性的重設密碼連結（含 token）。

> 典型使用時機：登入頁面點選「忘記密碼」後呼叫。
...
信件中會包含一組用於重設密碼的連結，格式通常如下：

* `http://localhost:3000/reset-password?token=<TOKEN>`

其中 `<TOKEN>` 為一次性且具有效期限的重設密碼 token，僅能使用一次。
...
**說明**：
使用者從信件連結取得 `token` 後，提交 `token` 與 `new_password` 以完成重設密碼。此 API 會驗證 token 是否存在、是否過期、是否已使用，通過後將使用者密碼更新為新密碼，並將 token 標記為已使用。
...
| 欄位             | 型別     |  必填 | 說明              |
| -------------- | ------ | :-: | --------------- |
| `token`        | string |  ✅  | 信件連結中的一次性 token |
| `new_password` | string |  ✅  | 新密碼（預設最少 8 碼）   |
...
```json
{ "data": null, "message": "密碼已成功重設。", "status": "ok" }
```
...
```json
{ "data": null, "message": "無效的重設連結。", "status": "error" }
```
...
```json
{ "data": null, "message": "此重設連結已被使用。", "status": "error" }
```
...
```json
{ "data": null, "message": "此重設連結已過期。", "status": "error" }
```

### Curl 範例

> 先從信件連結取得 `token=` 後面的值，再帶入以下 curl。
...
若你的登入 API 為 `POST /auth/session/`，可用新密碼登入確認：

```bash
curl -X POST "http://127.0.0.1:8000/auth/session/" \
  -H "Content-Type: application/json" \
  -d '{"username":"momomo","password":"NewPassword123!"}'
```
